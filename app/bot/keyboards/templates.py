...
def get_templates_selection_keyboard(
    ...
    for tpl in page_templates:
        ...
        if "shipping" in cat_lower:
            icon = "📦"
        elif "invoice" in cat_lower:
            icon = "💳"
        else:
            icon = "🧾"
        ...
        row.append(
            InlineKeyboardButton(
                f"{icon}  {tpl.name}", callback_data=CallbackDataHelper.encode("tpl:sel", submission_id, tpl.id)
            )
        )
        if len(row) == 2:
            rows.append(row)
    ...
    rows.append([
        InlineKeyboardButton("↩️  Back to Document", callback_data=CallbackDataHelper.encode("doc:rev", submission_id))
    ])
